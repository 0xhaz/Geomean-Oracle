...
import {GeomeanOracleImplementation} from "./shared/GeoMeanOracleImplementation.sol";
...
contract TestGeomeanORacle is Test, Deployers {
    ...
    function test_BeforeInitialize_AllowsPoolCreation() public {
        manager.initialize(key, SQRT_PRICE_1_1, ZERO_BYTES);
    }

    function test_BeforeInitialize_RevertsIfFee() public {
    ...
    function test_BeforeInitialize_Reverts_IfNot_MaxTickSpacing() public {
        vm.expectRevert(GeoMeanOracle.OnlyOneOraclePoolAllowed.selector);
        manager.initialize(
    ...
    function test_AfterInitialize_State() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
        GeoMeanOracle.ObservationState memory observationState;
        (observationState.index, observationState.cardinality, observationState.cardinalityNext) =
            geomeanOracle.states(key.toId());
        assertEq(observationState.index, 0);
        assertEq(observationState.cardinality, 1);
        assertEq(observationState.cardinalityNext, 1);
    }

    function test_AfterInitialize_Observation() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
    ...
    function test_AfterInitialize_Observe0() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
    ...
    function test_BeforeModifyPosition_NoObservations() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
    ...
    function test_BeforeModifyPosition_Observation() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
        geomeanOracle.setTime(3); // advance time to 3 seconds
        modifyLiquidityRouter.modifyLiquidity(
            key,
            IPoolManager.ModifyLiquidityParams(
                TickMath.minUsableTick(MAX_TICK_SPACING), TickMath.maxUsableTick(MAX_TICK_SPACING), 1000, 0
    ...
    function test_BeforeModifyPosition_ObservationAndCardinality() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
        geomeanOracle.setTime(3); // advance time to 3 seconds
        geomeanOracle.increaseCardinality(key, 2);
        GeoMeanOracle.ObservationState memory observationState;
        (observationState.index, observationState.cardinality, observationState.cardinalityNext) =
            geomeanOracle.states(key.toId());
        assertEq(observationState.index, 0);
    ...
    function test_Permanent_Liquidity() public {
        manager.initialize(key, SQRT_PRICE_2_1, ZERO_BYTES);
        geomeanOracle.setTime(3); // advance time to 3 seconds
        modifyLiquidityRouter.modifyLiquidity(
            key,
